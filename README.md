...
### Demo C：my-todo 复跑——系统抓到自己一个 bug（2026-09-05，tp 套餐端）

`runs/20260905-173115-588f` — **SUCCESS**，656s，9 次调用（输入 42.5k + 输出 27.8k token），修复轮 1/2。

- 前一轮（`runs/20260905-171558-8970`）真实 **FAILED**: 首测 2 用例挂 → fix#1 没修对 → fix#2 修对（21 用例全 PASS）→ review 却报阻断 → 轮次耗尽。根因是**系统自己的 bug**: TEST.md 机械结论是追加式，review 复审时读到历史 FAIL 段，把"已修复的旧失败"误判成"当前不稳定"
- 修复: 机械结论改为**覆盖式**（TEST.md 永远只带最新一次执行结果）+ 回归测试 → 复跑 SUCCESS，且这条跑还真实走完 arch 格式漂移重试、review 抓阻断 → code_fix（自身也无信封 → 重试）→ test 重跑 PASS → 复审干净
- 价值: 判定权分离让"系统抓自己的 bug"成为可能——FAILED 终态如实可审计，修完有回归测试钉死，重跑自证
- 另一次瞬时 STOPPED（`runs/20260905-172002-7bd9`）: spec 两轮格式漂移（无信封/未闭合），属 tp 端偶发质量波动，重跑即过——gate 重试机制存在的意义
